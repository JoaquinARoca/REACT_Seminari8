...
//Update a user
export const updateUser = async (email:string,updateUser:User):Promise<User> => {
    try{
        const response = await axios.put<User>('http://localhost:9000/api/users/email/'+email,updateUser);
        if (response.status !== 200) {
